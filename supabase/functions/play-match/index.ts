// play-match — the authoritative match resolver.
//
// Runs the SAME pure basic-game engine that lives in `src/game/engine/` (imported
// unchanged via the `@/` import map in deno.json), then commits the result through
// the `record_match` SECURITY DEFINER function using the service-role key. Because
// `record_match` is revoked from client roles, the scoreline and the coins it pays
// can only ever originate here — the browser can trigger a match but not forge one.

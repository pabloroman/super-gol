...
`npm run db:reset:linked` runs `seed.sql` against the **linked remote**, where a
published password on an `is_admin` account would be a real breach. The seed
guards against that itself and refuses to run anywhere but the local stack:
...
data. It sits one word away from `npm run db:reset` in `package.json`; the seed
guard above means it can no longer also plant a public admin password, but it will
still happily drop the database it's pointed at.
...
- **Production / any hosted DB** — imported through the app. After migrations apply,
  a hosted database comes up with an **empty `cards` table**. To fill it:
